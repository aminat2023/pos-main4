...
class CounterSales extends Component
{
    ...
    {
        if ($value) $this->addSelectedProductToCart((int) $value);
    }
    ...
    public function incrementQty(int $index): void
    {
        ...
        if ($row['quantity'] >= $row['max_quantity']) return;
    ...
    /* ------------- SAVE (deduct stock & record sale) ------------- */
    public function save(): void
    {
        if (!$this->orderItems) { $this->errorMessage = 'Cart is empty.'; return; }
