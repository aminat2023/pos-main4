                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form wire:submit.prevent="addSelectedProductToCart">
                            <div>
                                <input type="text" class="form-control" placeholder="Search product..." wire:model.debounce.300ms="searchTerm">
                            ...
                            <ul class="list-group">
                                @foreach ($recentReceipts as $receipt)
                                    <li class="list-group-item list-group-item-action"
                                        wire:click="showReceiptDetails('{{ $receipt['invoice_no'] }}')"
...
<div class="col-md-7">
    @if ($selectedReceiptItems && count($selectedReceiptItems))
        @foreach ($selectedReceiptItems as $item)
            <div class="mb-1">
                <strong>{{ $item['product_name'] }}</strong> (x{{ $item['quantity'] }}) - ₦{{ number_format($item['selling_price']) }}
            </div>
        @endforeach

        <div class="mt-3">
            <p><strong>Total:</strong> ₦{{ number_format($selectedReceiptAmount) }}</p>
            <p><strong>Change:</strong> ₦{{ number_format($selectedReceiptChange) }}</p>

            <!-- ✅ Print Button -->
            <button type="button" onclick="PrintReceiptContent('printOldReceipt')" class="btn btn-teal mt-3">
                <i class="fa fa-print"></i> Print This Receipt
            </button>
        </div>
    @else
        <p class="text-muted">Select a receipt to view details.</p>
    @endif
</div>
    ...
    <!-- Print container for selected old receipt -->
    <div id="printOldReceipt" style="display: none;">
        @includeIf('receipts.' . $receiptTemplate, [
            'orderItems' => $selectedReceiptItems,
            'pay_money' => $selectedReceiptAmount,
            'balance' => $selectedReceiptChange,
        ])
    </div>
</div>
...
<script>
    ...
    window.addEventListener('focusProductSelect', () => {
        document.querySelector('select[wire\\:model="selectedProductId"]').focus();
    });

    function showReceiptHistoryModal() {
        $('#receiptHistoryModal').modal('show');
    }
</script>
